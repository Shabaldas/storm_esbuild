describe '/dredd/product_categories', type: :request do
  ...
  describe 'GET /dredd/product_categories/new' do
    it 'display new product_category form' do
      get new_dredd_product_category_path
  ...
  describe 'GET /dredd/product_categories' do
    ...
    it 'display all product_categories' do
      get dredd_product_categories_path
      ...
      expect(response.body).to include(dredd_product_category_path(product_category_first))
      expect(response.body).to include(dredd_product_category_path(product_category_second))
      expect(response.body).to include(edit_dredd_product_category_path(product_category_first))
      expect(response.body).to include(edit_dredd_product_category_path(product_category_second))
  ...
  describe 'GET /dredd/product_category/:id' do
    context 'when the product category has children' do
      ...
      it 'display the product category' do
        get dredd_product_category_path(product_category)
  ...
  describe 'GET /dredd/product_categories/edit' do
    let(:product_category) { create(:product_category, name: 'Home') }

    it 'display the product category' do
      get edit_dredd_product_category_path(product_category)
  ...
  describe 'POST /dredd/product_categories' do
    context 'when valid params and parent empty' do
      it 'creates a new product_category' do
        expect do
          post dredd_product_categories_path, params: {
            product_category: {
              name: 'World',
        ...
        expect(response).to redirect_to(dredd_product_categories_path)
        follow_redirect!
  ...
  describe 'DELETE /dredd/product_categories/:id' do
    let!(:product_category) { create(:product_category, name: 'Home') }

    it 'destroy the product category' do
      expect do
        delete dredd_product_category_path(product_category)
      end.to change(ProductCategory, :count).by(-1)
      expect(response).to redirect_to(dredd_product_categories_path)
      follow_redirect!
